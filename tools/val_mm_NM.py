...
def add_salt_and_pepper_noise(image, density=0.2):
    """Add salt-and-pepper noise to an image."""
    noisy_image = image.clone()
    num_salt = np.ceil(density * image.numel() * 0.5).astype(int)
    num_pepper = np.ceil(density * image.numel() * 0.5).astype(int)

    # Salt noise
    coords = [torch.randint(0, i, (num_salt,)) for i in image.shape]
    noisy_image[coords] = image.max()

    # Pepper noise
    coords = [torch.randint(0, i, (num_pepper,)) for i in image.shape]
    noisy_image[coords] = image.min()
...
@torch.no_grad()
def evaluate_noisy_modality(model, dataloader, device):
    print('Evaluating...')
    model.eval()
    ...
    for images, labels in tqdm(dataloader):
        ...
        # Add salt-and-pepper noise to RGB, D, E, L
        images[0] = add_salt_and_pepper_noise(images[0], density=density[level]).to(device)  # RGB
        images[1] = add_salt_and_pepper_noise(images[1], density=density[level]).to(device) # D
        images[2] = add_salt_and_pepper_noise(images[2], density=density[level]).to(device)  # E
        images[3] = add_salt_and_pepper_noise(images[3], density=density[level]).to(device)  # L

        # Add Gaussian noise to RGB, D, L
        images[0] = add_gaussian_noise(images[0], mean=0.0, std=std[level]).to(device)  # RGB
        images[1] = add_gaussian_noise(images[1], mean=0.0, std=std[level]).to(device)  # D
        images[3] = add_gaussian_noise(images[3], mean=0.0, std=std[level]).to(device) # L'''
    ...
    for case in cases:
        ...
        if True:
            if eval_cfg['MSF']['ENABLE']:
                ...
                acc, macc, f1, mf1, ious, miou = evaluate_noisy_modality(model, dataloader, device)
