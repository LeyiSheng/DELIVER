...
@torch.no_grad()
def evaluate_with_random_missing_modalities(model, dataloader, device):
    print('Evaluating with random_missing modalities...')
    model.eval()
    n_classes = dataloader.dataset.n_classes
        ...
        for missing_modalities in itertools.combinations(modalities, num_missing):
            ...
            for images, labels in tqdm(dataloader):
                ...
                for modality in missing_modalities:
                    ...
                    print(f'Original image shape for modality {modality}: {image.shape}')'''
                    # Calculate the total number of pixels in the image
                    # 使用随机生成的布尔数组来选择要置零的像素
                    mask = torch.rand(image.shape) < r  # 生成一个与图像形状相同的布尔数组
                    image[mask] = 0  # 使用布尔索引将对应的像素置为零
                    '''print(f'Modality {modality} has {num_pixels_to_zero} pixels set to zero.')
                    if not torch.equal(image, original_image):
                        print(f'Modality {modality}: Image has been modified.')
                    else:
                        print(f'Modality {modality}: Image is unchanged (this should not happen).')
                    print(f'Current image shape for modality {modality}: {image.shape}')'''
    ...
    for case in cases:
        ...
        if True:
            ...
            if eval_cfg['MSF']['ENABLE']:
                ...
                results = evaluate_with_random_missing_modalities(model, dataloader, device)
                ...
                for missing_modalities, metrics in results.items():
                    table['Random Missing Modalities'].append(str(missing_modalities))
